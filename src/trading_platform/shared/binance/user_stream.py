...
class UserDataStream:
            ...
            try:
                ...
                if event_type == 'ORDER_TRADE_UPDATE':
                    ...
                    if self.on_execution_report:
                        # 在事件循环中调度回调
                        asyncio.create_task(self._handle_execution_report(order_data))
    ...
    async def _handle_execution_report(self, order_data: dict[str, Any]) -> None:
        ...
        try:
            # 异步调用回调
            if asyncio.iscoroutinefunction(self.on_execution_report):
                await self.on_execution_report(order_data)
            else:
                self.on_execution_report(order_data)
        except Exception as e:
