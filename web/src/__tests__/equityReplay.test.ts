...
function trade(overrides: Partial<BacktestEquityTrade>): BacktestEquityTrade {
  return {
    id: String(overrides.id || 'trade'), symbol: String(overrides.symbol || 'BTCUSDT'),
    entry_time: overrides.entry_time ?? 1_000, exit_time: overrides.exit_time ?? 2_000,
    entry_price: overrides.entry_price ?? 100, exit_price: overrides.exit_price ?? 90,
    net_pnl: overrides.net_pnl ?? 100, gross_pnl: overrides.gross_pnl ?? 100,
    entry_notional: overrides.entry_notional ?? 1_000, ...overrides
  }
}
