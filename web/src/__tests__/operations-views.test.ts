...
describe('operations views', () => {
  ...
  it('performance consumes authoritative breakdown rows in Shanghai time', async () => {
    ...
    vi.spyOn(operationsApi, 'performance').mockResolvedValue({
      ...
      total_realized_pnl: '5.1',
      net_pnl: '5',
      max_drawdown: '0',
      candidate_campaigns: 1,
    ...
    const breakdown = vi.spyOn(operationsApi, 'performanceBreakdown').mockResolvedValue({
      ...
      items: [
        {
          dimension_key: 'BTCUSDT',
          dimension_label: 'BTCUSDT',
          total_trades: 1,
          total_fills: 2,
          win_count: 1,
          loss_count: 0,
  ...
  it('trade review sends calendar drill-down dates to the backend', async () => {
    ...
    const campaigns = vi
      .spyOn(operationsApi, 'campaigns')
      .mockResolvedValue({ items: [], total: 200, limit: 50, offset: 100, unattributed_fills: 0 })

    mount(TradeReviewView)
    await flushPromises()

    expect(campaigns).toHaveBeenCalledWith(
      expect.objectContaining({
        account_id: 'acct',
        start_date: '2026-08-01',
        end_date: '2026-08-01',
      ...
      items: [
        {
          ...
          strategy_id: 's',
          symbol: 'BTCUSDT',
          campaign_id: 'campaign-with-a-long-identifier',
          side: 'SHORT',
          fill_count: 2,
          sell_quantity: '1',
          buy_quantity: '1',
          ...
          net_realized_pnl: '9.9',
          first_fill_at: '2026-08-01T00:00:00Z',
          last_fill_at: '2026-08-01T00:01:00Z',
          closed_at: '2026-08-01T00:01:00Z',
          has_open_quantity: false,
          pnl_facts_complete: true,
        },
      ],
      total: 1,
      limit: 50,
  ...
  it('clears the slotted trade-date filter together with common filters', async () => {
    ...
    const campaigns = vi
      .spyOn(operationsApi, 'campaigns')
      .mockResolvedValue({ items: [], total: 0, limit: 50, offset: 0, unattributed_fills: 0 })
  ...
  it('loads Campaign detail and maps every ledger fill to the shared chart', async () => {
    ...
    const campaignPnl = vi.spyOn(operationsApi, 'campaignPnl').mockResolvedValue({
      ...
      commission_asset: 'USDT',
      gross_realized_pnl: '5',
      net_realized_pnl: '4.8',
      remaining_quantity: '0',
      has_open_quantity: false,
      acquired_at: null,
      first_fill_at: '2026-08-01T00:00:00Z',
      last_fill_at: '2026-08-01T00:02:00Z',
      closed_at: '2026-08-01T00:02:00Z',
      released_at: null,
      lifecycle_duration_ms: 120000,
    })
      ...
      items: [
        {
          ...
          exchange_time: '2026-08-01T00:01:00Z',
        },
        {
          id: 1,
          account_id: 'acct',
          ...
          is_maker: false,
          created_at: '2026-08-01T00:00:00Z',
          exchange_time: '2026-08-01T00:00:00Z',
        },
      ],
      total: 2,
      limit: 1000,
    ...
    const dailyPnl = vi.spyOn(operationsApi, 'dailyPnl').mockResolvedValue([
      {
        ...
        gross_realized_pnl: '10.2',
        total_commission: '0.2',
        commission_asset: 'USDT',
        net_pnl: '10',
        funding_fee: null,
        net_pnl_scope: 'realized PnL minus USDT commission; funding fee facts unavailable',
      },
    ])
  ...
  it('loads unclassified symbols from the backend and marks a failed sync', async () => {
    ...
    vi.spyOn(operationsApi, 'symbolSyncStatus').mockResolvedValue({
      initialized: true,
      status: 'FAILED',
    ...
    const exchangeSymbols = vi.spyOn(operationsApi, 'exchangeSymbols').mockImplementation(async (query) => ({
      items: [
        {
          symbol: query?.offset ? 'PAGE2USDT' : 'ORPHANUSDT',
          pair: 'ORPHANUSDT',
  ...
  it('loads every exchange-symbol page before applying local filters', async () => {
    ...
    const exchangeSymbols = vi
      .spyOn(operationsApi, 'exchangeSymbols')
  ...
  it('strategy risk renders effective-universe reasons from the backend response', async () => {
    ...
    const universePreview = vi
      .spyOn(operationsApi, 'universePreview')
      .mockResolvedValueOnce({
        ...
        excluded_symbols: 0,
        total: 2,
        items: [{ symbol: 'BTCUSDT', effective: true, exclusion_reasons: [], blocked_category_keys: [] }],
        limit: 1000,
        offset: 0,
      })
      .mockResolvedValueOnce({
        ...
        excluded_symbols: 0,
        total: 2,
        items: [{ symbol: 'ETHUSDT', effective: true, exclusion_reasons: [], blocked_category_keys: [] }],
        limit: 1000,
        offset: 1,
      })
    ...
    expect(universePreview).toHaveBeenNthCalledWith(
      2,
      'spike-short',
      expect.objectContaining({ limit: 1000, offset: 1 }),
    )
  })
