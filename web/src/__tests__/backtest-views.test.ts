...
describe('回测关键视图', () => {
  ...
  it('单笔复盘按1500根窗口加载，并在退出定位时重新以退出时间取数', async () => {
    ...
    expect(wrapper.find('.event-groups').exists()).toBe(true)
    expect(wrapper.findAll('.event-group-card').length).toBe(2)
    expect(wrapper.text()).toContain('价格与标线')
    expect(wrapper.text()).toContain('涨幅与量比')
    expect(wrapper.findAll('.event-parameters tbody tr').length).toBe(3)
    expect(wrapper.findAll('.event-parameters thead th').length).toBe(6)
    ...
    expect(wrapper.find('.event-groups').text()).toContain('5 秒涨幅门槛：5.00%')
