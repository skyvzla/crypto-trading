...
const columns: TableColumnsType<BacktestSymbolSummary> = [
  {
    ...
    width: 150,
    sorter: true,
    customRender: ({ record: row }) => h('strong', { class: 'symbol-name' }, row.symbol),
  },
  ...
  },
  {
    title: '平均盈利',
    key: 'average_win',
    dataIndex: 'average_win',
    width: 110,
    sorter: true,
    customRender: ({ record: row }) => formatNumber(row.average_win),
  },
  {
    title: '平均亏损',
    key: 'average_loss',
    dataIndex: 'average_loss',
    width: 110,
    sorter: true,
    customRender: ({ record: row }) => formatNumber(row.average_loss),
  },
  {
    title: '平均持仓',
    key: 'average_holding_seconds',
    dataIndex: 'average_holding_seconds',
    width: 110,
    sorter: true,
  ...
  },
  {
    ...
    customRender: ({ record: row }) =>
      h(Tooltip, { title: '查看交易记录' }, () =>
        h(
          RouterLink,
          {
            to: {
              path: `/backtests/${encodeURIComponent(researchId.value)}/symbols/${encodeURIComponent(row.symbol)}/trades`,
              query: preservedQuery.value,
            },
          },
          () =>
            h(Button, { type: 'text', shape: 'circle', 'aria-label': '查看交易记录' }, () =>
...
<template>
  <BacktestPage
    ...
    :eyebrow="researchId"
    :back-to="rootTo"
    :crumbs="[{ label: '回测复盘', to: rootTo }, { label: '交易对数据' }]"
  >
    <div class="symbol-table-tools">
      <a-input-search
        v-model:value="symbolInput"
        allow-clear
        placeholder="筛选交易对，例如 AKE 或 USDT"
        enter-button="筛选"
        @search="onSearch"
      />
    ...
    >
      <div class="table-frame">
        ...
        <div class="pagination-bar">
          <span>共 {{ query.data.value?.total || 0 }} 个交易对</span
          ><a-pagination
