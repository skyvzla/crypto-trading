...
const columns: TableColumnsType<BacktestSymbolSummary> = [
  ...
  { title: '', key: 'action', width: 64, fixed: 'right', customRender: ({ record: row }) => h(Tooltip, { title: '查看交易记录' }, () => h(RouterLink, { to: `/backtests/${encodeURIComponent(researchId.value)}/symbols/${encodeURIComponent(row.symbol)}/trades` }, () => h(Button, { type: 'text', shape: 'circle', 'aria-label': '查看交易记录' }, () => h(ArrowRight, { size: 16 })))) }
]
</script>
