...
const query = useQuery({
  queryKey: computed(() => ['backtest-reports', researchId.value]),
  queryFn: () => backtestApi.reports(researchId.value),
        ...
        <article v-for="report in query.data.value?.items" :key="report.type" class="catalog-row">
          ...
          <div class="catalog-copy">
            <div>
              ...
              ><a-tag v-if="report.category" color="blue">{{ report.category }}</a-tag>
            </div>
          ...
          <RouterLink
            :to="{
              path: `/backtests/${encodeURIComponent(researchId)}/reports/${encodeURIComponent(report.type)}`,
              query: route.query,
            }"
          >
            <a-button
              >打开<template #icon><ArrowRight :size="16" /></template
            ></a-button>
          </RouterLink>
        </article>
      </div>
