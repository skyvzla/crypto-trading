...
const columns = computed<TableColumnsType<JsonObject>>(() =>
  (query.data.value?.columns || []).map((item) => {
    const column: ReportColumn = typeof item === 'string' ? { key: item } : item
    const providedTitle = column.title || column.label || ''
    const label = /[\u4e00-\u9fff]/.test(providedTitle) ? providedTitle : reportLabel(column.key)
    return {
      title: h('div', { class: 'report-header' }, [
        h('span', { class: 'report-header-label' }, label),
        h('code', { class: 'report-header-key' }, column.key),
      ]),
      key: column.key,
      dataIndex: column.key,
      width: 150,
      sorter: column.sortable === false ? undefined : true,
      customRender: ({ record: row }) =>
        h('span', { class: 'report-value' }, displayValue(row[column.key] as JsonValue, column.type)),
    }
  }),
)
function onTableChange(
  _: unknown,
  __: unknown,
  sorter: { field?: string; order?: string } | Array<{ field?: string; order?: string }>,
) {
...
<template>
  <BacktestPage
    :title="query.data.value?.descriptor.title || reportType"
    :eyebrow="reportType"
    :back-to="backTo"
    :crumbs="[
      { label: '回测复盘', to: rootTo },
      { label: '分析报表', to: backTo },
      { label: query.data.value?.descriptor.title || reportType },
    ]"
  >
    ...
    >
      <div class="table-frame">
        <a-table
          :columns="columns"
          :data-source="query.data.value?.rows || []"
          row-key="trade_id"
        ...
        <div class="pagination-bar">
          <span>共 {{ query.data.value?.total || 0 }} 行</span
          ><a-pagination
