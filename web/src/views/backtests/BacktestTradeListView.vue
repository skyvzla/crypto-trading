...
const minPnl = ref<number | null>(typeof route.query.min_pnl === 'string' && Number.isFinite(Number(route.query.min_pnl)) ? Number(route.query.min_pnl) : null)
const maxPnl = ref<number | null>(typeof route.query.max_pnl === 'string' && Number.isFinite(Number(route.query.max_pnl)) ? Number(route.query.max_pnl) : null)
...
const columns: TableColumnsType<BacktestTradeSummary> = [
  ...
  { title: '净盈亏', key: 'net_pnl', dataIndex: 'net_pnl', width: 110, sorter: true, customRender: ({ record: row }) => h('span', { class: pnlClass(row.net_pnl) }, formatNumber(row.net_pnl)) },
  { title: '收益率', key: 'net_return', dataIndex: 'net_return', width: 90, sorter: true, customRender: ({ record: row }) => formatPercent(row.net_return) },
  { title: '结果', key: 'winner', dataIndex: 'winner', width: 74, sorter: true, customRender: ({ record: row }) => h(Tag, { color: row.winner ? 'success' : 'error' }, () => row.winner ? '盈利' : '亏损') },
  { title: '退出原因', key: 'exit_reason', dataIndex: 'exit_reason', minWidth: 170, sorter: true },
  { title: '', key: 'action', width: 64, fixed: 'right', customRender: ({ record: row }) => h(Tooltip, { title: '打开 K 线复盘' }, () => h(RouterLink, { to: { path: `/backtests/${encodeURIComponent(researchId.value)}/trades/${encodeURIComponent(row.id)}`, query: preservedQuery.value } }, () => h(Button, { type: 'text', shape: 'circle', 'aria-label': '打开K线复盘' }, () => h(CandlestickChart, { size: 16 })))) }
]
</script>

<template>
    <BacktestPage :title="`${symbol} 交易记录`" :eyebrow="researchId" :back-to="backTo" :crumbs="[{ label: '回测复盘', to: rootTo }, { label: '交易对数据', to: backTo }, { label: symbol }]">
    <QueryPanel :pending="query.isPending.value" :error="query.error.value" :empty="query.data.value?.items.length === 0" @retry="query.refetch()">
      <div class="trade-table-tools">
        <a-select v-model:value="resultFilter" :options="[{ value: 'all', label: '全部结果' }, { value: 'win', label: '仅盈利' }, { value: 'loss', label: '仅亏损' }]" />
