...
const FIELD_LABELS: Record<string, string> = {
  ...
  rise_15s: '15 秒涨幅',
  rise_60s: '60 秒涨幅',
  rise_threshold_5s: '5 秒涨幅门槛',
  volume_5s: '5 秒成交量',
  median_volume_1s: '1 秒成交量中位数',
  volume_multiple_5s: '5 秒量比',
  volume_threshold_5s: '5 秒量比门槛',
  rise_from_12h_low: '相对 12 小时低点涨幅',
  td_sell_setup_5m: '5 分钟 TD 卖出序列',
  td_sell_setup_15m: '15 分钟 TD 卖出序列',
  upper_wick_ratio_5m: '5 分钟上影线比例',
  upper_wick_ratio_15m: '15 分钟上影线比例',
  volume_multiple_5m: '5 分钟量比',
  spike_high: '尖峰高点',
  spike_high_time: '尖峰高点时间',
  origin_price: '起涨价格',
  origin_floor: '起涨价格下限',
  rise_low: '上涨窗口低点',
  invalid_price: '失效价格',
  pullback_threshold: '回调触发价',
  pullback_atr: '回调 ATR 倍数',
  tier_prices: '分档价格',
  tier_weights: '分档权重',
  active_time: '计划生效时间',
  expire_time: '计划失效时间',
  entry_pattern: '入场形态',
  pullback_before_fill: '成交前已回调',
  pullback_time: '回调时间',
  pullback_low: '回调低点',
  mark_price: '标记价格',
  gross_pnl: '毛盈亏',
  ...
  action: '执行动作',
  reason: '原因',
  rejection_stage: '拒绝环节',
  rejection_reasons: '拒绝原因',
  cancelled_orders: '已撤订单数',
  order_id: '订单 ID',
  fill_id: '成交 ID',
  quantity: '数量',
  price: '成交价格',
  first_fill_time: '首笔成交时间',
  origin_signal_time: '原信号时间',
  entry_tier_mode: '入场分档模式',
  exit_policy: '退出策略',
  strategy_version: '策略版本',
  scored_score: '准入评分',
  scored_threshold: '准入评分门槛',
  max_rise_5s: '5 秒最大涨幅',
  ...
  previous_oi: '前值持仓量',
  oi_change_pct: '持仓量变化',
  max_oi_change_pct: '持仓量变化上限',
  ls_ratio: '多空账户比',
  max_ls_ratio: '多空账户比上限',
  min_td_sell_setup_5m: '5 分钟 TD 卖出序列下限',
  min_volume_multiple_5m: '5 分钟量比下限',
  metrics_available_time: '指标可用时间',
  d_oi_pct: '确认期持仓量涨幅',
  oi_stop_oi_rise_pct: '持仓量止损涨幅门槛',
  loss_pct: '价格亏损幅度',
  oi_stop_loss_pct: '持仓量止损亏损门槛',
  hard_stop_loss_ratio: '硬止损实际亏损',
  hard_stop_loss_pct: '硬止损门槛',
  hard_stop_confirm_ms: '硬止损确认时长',
  candidate: '候选入场价',
  entry_price: '计划入场价',
  impulse_base_price: '脉冲起点价格',
  retrace_frac: '回撤比例',
  bar_low: '当前 K 线低点',
  buy_ratio_entry: '主动买入占比',
  buy_ratio_entry_min: '主动买入占比下限',
  wait_ms: '等待时长',
  timeout_stage: '超时阶段',
  observed_close: '观察收盘价',
  target_price: '目标价格',
  pnl_pct: '盈亏幅度',
  base_ms: '持仓量基准时间',
  confirm_ms: '持仓量确认时间',
  rise_60s_threshold: '60 秒涨幅门槛',
  decision: '决策',
  decay_agreement: '动能衰减一致度',
}
...
const VOLUME_FIELDS = /(?:^|_)volume(?:_|$)/
const IDENTIFIER_FIELDS = /(?:^|_)(?:id|version)(?:_|$)/
...
const CODE_VALUE_LABELS: Record<string, string> = {
  ...
  time_risk: '时间风险',
  momentum_risk: '动量风险',
  profit_drawdown: '盈利回撤',
  gate_stop: '门禁止损',
  trend_exit: '趋势退出',
  hold: '继续持有',
  exit: '退出',
  reduce_half: '减半仓位',
  origin_momentum_continues: '起涨点动能延续',
  origin_momentum_decay: '起涨点动能衰减',
...
export function resolvePricePrecision(
  ...
  const prices = samplePrices
    .map((price) => Math.abs(Number(price)))
    .filter((price) => Number.isFinite(price) && price > 0)
  if (!prices.length) return 2

  const representativePrice = prices.sort((left, right) => left - right)[Math.floor(prices.length / 2)]
  if (representativePrice >= 100) return 2
  if (representativePrice >= 1) return 4
  if (representativePrice >= 0.1) return 5
  if (representativePrice >= 0.01) return 6
  if (representativePrice >= 0.001) return 7
  if (representativePrice > 0) return 8
  return 2
}
...
function formatCompactMetric(value: number): string {
  ...
  return unitIndex > 0 ? `${formatNumber(scaled, 2)}${units[unitIndex]}` : formatNumber(value, 3)
}
...
export function formatEventValue(key: string, value: unknown, pricePrecision?: number): string {
  ...
  if (Array.isArray(value))
    return CODE_VALUE_FIELDS.has(key)
  ...
  if (typeof value === 'object') {
    return (
      Object.entries(value as Record<string, unknown>)
        .map(
          ([childKey, childValue]) =>
            `${eventParameterLabel(childKey)}：${formatEventValue(childKey, childValue, pricePrecision)}`,
        )
        .join('；') || '-'
    )
  }
  if (CODE_VALUE_FIELDS.has(key)) return formatCodeValue(value)
  if ((TIME_FIELDS.test(key) || key === 'base_ms' || key === 'confirm_ms') && Number.isFinite(Number(value)))
  ...
  if (PNL_FIELDS.test(key) && Number.isFinite(Number(value))) return formatNumber(Number(value), 3)
  if (QUANTITY_FIELDS.test(key) && Number.isFinite(Number(value))) return formatNumber(Number(value), 6)
  if ((VOLUME_FIELDS.test(key) || key === 'oi' || key === 'previous_oi') && Number.isFinite(Number(value)))
    return formatCompactMetric(Number(value))
  if (IDENTIFIER_FIELDS.test(key)) return String(value)
...
export function eventParameterRows(
  ...
  if (event.price != null && data.price == null && data.trigger_price == null && data.fill_price == null)
  ...
  const rows = Object.entries(data)
    .map(([key, value], sourceIndex) => {
      ...
      return {
        key,
        label:
          key === 'price' && event.type?.includes('exit_requested')
            ? '退出触发价格'
