...
const parameterOptions = computed(() =>
  (parameterSetsQuery.data.value?.items || []).map((item, index) => ({
...
const parsedParameters = computed<Record<string, unknown>>(() => {
  try {
    return selectedParameters.value ? JSON.parse(selectedParameters.value) : {}
  } catch {
    return {}
  }
})
...
const result = computed(() =>
  replayEquity(tradesQuery.data.value?.items || [], {
...
const columns: TableColumnsType<EquityReplayRow> = [
  { title: '#', key: 'sequence', dataIndex: 'sequence', width: 60 },
  {
    ...
    dataIndex: 'symbol',
    width: 110,
    customRender: ({ text }) => h('strong', { class: 'symbol-name' }, String(text)),
  },
  {
    title: '入场时间',
    key: 'entry_time',
  ...
  },
  {
    ...
    width: 110,
    customRender: ({ record }) =>
      h(Tag, { color: record.status === 'executed' ? 'success' : 'default' }, () =>
        record.status === 'executed' ? '已执行' : skipLabels[record.skipReason!] || '已忽略',
      ),
  },
  {
    title: '本笔仓位',
    ...
    width: 110,
    customRender: ({ text }) => `${formatNumber(text)} U`,
  },
  {
    title: '毛收益率',
    key: 'grossReturn',
  ...
  },
  {
    ...
    width: 110,
    customRender: ({ record }) =>
      h('span', { class: pnlClass(record.replayPnl) }, `${formatNumber(record.replayPnl)} U`),
  },
  {
    title: '本笔复投',
    key: 'reinvestedProfit',
    dataIndex: 'reinvestedProfit',
    width: 110,
    customRender: ({ text }) => `${formatNumber(text)} U`,
  },
  {
    title: '交易资金池',
    key: 'tradingCapitalAfter',
    dataIndex: 'tradingCapitalAfter',
    width: 120,
    customRender: ({ text }) => `${formatNumber(text)} U`,
  },
  {
    title: '锁定储备',
    key: 'reserveCapitalAfter',
    dataIndex: 'reserveCapitalAfter',
    width: 110,
    customRender: ({ text }) => `${formatNumber(text)} U`,
  },
  {
    ...
    customRender: ({ record: row }) =>
      h(Tooltip, { title: '打开单笔 K 线复盘' }, () =>
        h(
          ...
          () =>
            h(Button, { type: 'text', shape: 'circle', 'aria-label': '打开单笔K线复盘' }, () =>
...
<template>
  ...
  >
    <QueryPanel
      ...
      @retry="parameterSetsQuery.refetch()"
    >
      <section class="equity-controls" aria-label="回放参数">
        <label class="parameter-choice"
          ><span>策略参数</span><a-select v-model:value="selectedParameters" :options="parameterOptions"
        /></label>
        <div class="equity-value-fields">
          <label
            ><span>初始资金</span
            ><a-input-number v-model:value="initialBalance" :min="0.01" :precision="2" addon-after="U"
          /></label>
          <label
            ><span>初始仓位</span
            ><a-input-number
              v-model:value="initialPosition"
              :min="0"
              :max="initialBalance"
              :precision="2"
              addon-after="U"
          ...
          <label
            ><span>盈利复投</span
            ><a-input-number v-model:value="reinvestPercent" :min="0" :max="100" :precision="2" addon-after="%"
          /></label>
          <label
            ><span>最低交易资金池</span
            ><a-input-number v-model:value="minimumBalance" :min="0" :precision="2" addon-after="U"
          /></label>
          <label
            ><span>单边手续费</span><a-input-number v-model:value="feePercent" :min="0" :precision="4" addon-after="%"
          /></label>
          <label
            ><span>单边滑点</span
            ><a-input-number v-model:value="slippagePercent" :min="0" :precision="4" addon-after="%"
          /></label>
        </div>
        <div class="equity-reset">
          <a-tooltip title="恢复默认参数"
            ><a-button type="text" shape="circle" aria-label="恢复默认回放参数" @click="resetSettings"
              ><RotateCcw :size="16" /></a-button
          ></a-tooltip>
        </div>
      </section>
      ...
      >
        <div class="equity-summary-strip">
          <div>
            <span>最终权益</span><strong>{{ formatNumber(result.finalBalance) }} U</strong>
          </div>
          <div>
            <span>净收益</span
            ><strong :class="pnlClass(result.netProfit)">{{ formatNumber(result.netProfit) }} U</strong>
          </div>
          <div>
            <span>总收益率</span
            ><strong :class="pnlClass(result.returnRate)">{{ formatPercent(result.returnRate) }}</strong>
          </div>
          <div>
            <span>最大回撤</span><strong class="value-negative">{{ formatPercent(result.maxDrawdown) }}</strong>
          </div>
          <div>
            <span>交易资金池</span><strong>{{ formatNumber(result.finalTradingCapital) }} U</strong>
          </div>
          <div>
            <span>锁定储备</span><strong>{{ formatNumber(result.finalReserveCapital) }} U</strong>
          </div>
          <div>
            <span>执行 / 忽略</span><strong>{{ result.executedCount }} / {{ result.skippedCount }}</strong>
          </div>
          <div>
            <span>账户状态</span
            ><strong :class="result.liquidated ? 'value-negative' : 'value-positive'">{{
              result.liquidated ? '已停止' : '运行完成'
            }}</strong>
          </div>
        </div>

        <section class="equity-chart-section">
          <div class="equity-section-heading">
            <h3>权益走势</h3>
            <span>平仓结算 · 成交点可悬停</span>
          </div>
