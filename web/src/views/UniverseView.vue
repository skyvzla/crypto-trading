...
const categoryOptions = computed(() =>
  categories.value.map((item) => ({
    label: `${item.category_type === 'SUBCATEGORY' ? '└ ' : ''}${item.name} (${item.symbol_count})`,
...
const columns: TableColumnsType<ExchangeSymbol> = [
  {
    ...
    width: 145,
    customRender: ({ record }) =>
      h(Button, { type: 'link', class: 'table-link', onClick: () => openDetail(record) }, () => record.symbol),
  },
  {
    title: '基础 / 计价',
    key: 'assets',
    width: 145,
    customRender: ({ record }) => `${record.base_asset || '—'} / ${record.quote_asset || '—'}`,
  },
  {
    title: '合约',
    ...
    key: 'contract',
    width: 130,
    customRender: ({ text }) => h(Tag, { color: 'blue' }, () => String(text)),
  },
  {
    title: '交易所状态',
    dataIndex: 'status',
    key: 'status',
    width: 130,
    customRender: ({ text }) => h(Tag, { color: text === 'TRADING' ? 'green' : 'gold' }, () => String(text)),
  },
  {
    title: 'Category',
    dataIndex: 'underlying_type',
    key: 'category',
    width: 135,
  ...
  {
    title: '全局准入',
    dataIndex: 'global_enabled',
    key: 'admission',
    width: 115,
    customRender: ({ record }) =>
      h(Switch, {
        checked: record.global_enabled,
        loading: updating.value.has(record.symbol),
        checkedChildren: '允许',
        unCheckedChildren: '禁止',
        onChange: (value) => requestAdmissionChange(record, Boolean(value)),
      }),
  },
  {
    title: '同步时间',
...
async function saveAdmission() {
  ...
  try {
    const updated = await operationsApi.updateSymbolAdmission(item.symbol, {
...
async function openDetail(item: ExchangeSymbol) {
  ...
  const [categoryResult, auditResult] = await Promise.allSettled([
    operationsApi.symbolCategories(item.symbol),
    loadAudits(item.symbol),
  ])
...
<template>
  <main class="operations-page universe-page">
    <PageHeader
      eyebrow="REFERENCE DATA / GLOBAL GATE"
      title="交易对管理"
      description="只维护交易所标的事实与平台全局准入；策略分类开关已迁移到策略风控。"
      :loading="loading"
    ...
    >
      <DatabaseBackup :size="14" /><span
        >同步状态 <strong>{{ syncStatus.status }}</strong> · 最近成功 {{ formatDateTime(syncStatus.last_success_at) }} ·
        {{ syncStatus.synced_symbols }} 个交易对 · effective universe
        {{ syncStatus.effective_universe_ready ? 'ready' : 'not ready' }}</span
      ><a-tag v-if="syncStatus.stale" color="gold">STALE</a-tag
      ><span v-if="syncStatus.last_error">{{ syncStatus.last_error }}</span>
    </div>
    ...
    <a-modal
      ...
      :title="`${pendingEnabled ? '允许' : '禁止'} ${pendingSymbol?.symbol} 全局准入`"
      ok-text="确认修改"
    ...
    >
      ...
      <label class="reason-field"
        ><span>修改原因 *</span
        ><a-textarea v-model:value="reason" :rows="3" maxlength="500" show-count placeholder="记录本次全局准入变更原因"
      /></label>
    </a-modal>
    ...
    <a-drawer v-model:open="detailOpen" width="min(760px, 96vw)" :title="`${selected?.symbol || ''} · 交易对详情`">
      <template v-if="selected">
        <a-descriptions :column="2" bordered size="small">
          <a-descriptions-item label="基础 / 计价"
            >{{ selected.base_asset || '—' }} / {{ selected.quote_asset || '—' }}</a-descriptions-item
          >
          ...
          <a-descriptions-item label="全局准入"
            ><a-tag :color="selected.global_enabled ? 'green' : 'red'">{{
              selected.global_enabled ? '允许' : '禁止'
            }}</a-tag>
            v{{ selected.global_admission_version }}</a-descriptions-item
          >
        </a-descriptions>
        <section class="drawer-section">
          ...
          <a-space wrap
            ><a-tag
              ...
              :color="category.category_type === 'CATEGORY' ? 'blue' : undefined"
              >{{ category.name }} · {{ category.category_type }}</a-tag
            ><span v-if="!selectedCategories.length" class="muted">没有同步分类关联</span></a-space
          >
        </section>
        <a-alert
          class="metadata-note"
          type="info"
          show-icon
          message="当前交易对接口未返回精度、最小下单量等 metadata，因此详情不使用占位值冒充事实。"
        />
        <section class="drawer-section">
          <h3>全局准入审计</h3>
            ...
            <table class="audit-table">
              ...
              <tbody>
                <tr v-for="audit in audits" :key="audit.id">
                  ...
                  <td>
                    {{ audit.previous_enabled == null ? '默认' : audit.previous_enabled ? '允许' : '禁止' }} →
                    {{ audit.enabled ? '允许' : '禁止' }}
                  </td>
          ...
          <div v-else class="muted">没有显式变更记录，当前为默认允许。</div>
        </section>
      </template>
    </a-drawer>
  </main>
...
.universe-filters {
  display: grid;
  grid-template-columns: minmax(180px, 1.3fr) minmax(130px, 0.6fr) minmax(120px, 0.6fr) minmax(220px, 1fr) auto;
  align-items: center;
  gap: 8px;
  margin-bottom: 12px;
}
.universe-filters .ant-select {
  width: 100%;
}
.universe-filters > span {
  color: var(--muted);
