...
const trees = computed(() =>
  categories.value
    .filter((item) => item.category_type === 'CATEGORY')
    .map((parent) => ({
      ...parent,
      children: categories.value.filter((child) => child.parent_key === parent.category_key),
    })),
)
const visibleTrees = computed(() => {
  const query = categorySearch.value.trim().toLowerCase()
  if (!query) return trees.value
  return trees.value.filter((item) =>
    [item.name, item.code, ...item.children.flatMap((child) => [child.name, child.code])].some((value) =>
...
function enabled(item: ExchangeCategory): boolean {
  return admissions.value.get(item.category_key)?.enabled ?? defaultEnabled(item)
}
  ...
  async ({ isStale }) => {
    ...
    if (!strategies.value.length) {
      ...
      expanded.value = new Set(
        categoryRows.filter((item) => item.category_type === 'CATEGORY').map((item) => item.category_key),
      )
    ...
    const [rules, universe, auditPage] = await Promise.all([
      collectPageItems((params) => operationsApi.strategyAdmissionsPage(strategyId.value, params)).then(
        (page) => page.items,
      ),
...
async function saveChange() {
  ...
  try {
    await operationsApi.updateStrategyAdmission(strategyId.value, pendingCategory.value.category_key, {
...
function reasons(item: UniversePreviewItem): string {
  return item.exclusion_reasons.length ? item.exclusion_reasons.join('；') : '通过交易所、全局与策略分类门禁'
}
</script>
...
<template>
  <main class="operations-page strategy-risk-page">
    <PageHeader
      eyebrow="RISK / CATEGORY ADMISSION"
      title="策略风控"
      description="策略分类默认仅允许 COIN，其余分类关闭；可按策略显式放行或关闭 Category/Subcategory，不控制策略进程或交易参数。"
      :loading="loading"
    ...
    <div class="strategy-selector">
      ...
      /></label>
      <div><ShieldCheck :size="15" /><span>策略来源：账本 runtime status</span></div>
    </div>
    <a-alert
      v-if="!strategies.length && !loading"
      type="warning"
      show-icon
      message="没有可选择的策略"
    ...
    <DataState v-if="strategyId" :loading="loading" :error="error" @retry="reload">
      <section v-if="preview" class="metric-grid preview-metrics">
        <MetricTile
          label="候选交易对"
          :value="String(preview.total_symbols)"
          :hint="`交易所生命周期 + freeze_days=${UNIVERSE_FREEZE_DAYS}`"
        />
        <MetricTile
          label="最终有效交易池"
          :value="String(preview.effective_symbols)"
          tone="positive"
          hint="与执行 worker 同源判定"
        />
        <MetricTile
          label="已排除交易对"
          :value="String(preview.excluded_symbols)"
          :tone="preview.excluded_symbols ? 'warning' : 'neutral'"
          hint="含上游全局与策略门禁"
        />
        <MetricTile
          label="当前关闭分类"
          :value="String(categories.filter((item) => !enabled(item)).length)"
          hint="默认仅允许 COIN；显式配置优先"
        />
      </section>
      <div class="risk-rule">
        <GitBranch :size="15" /><span
          >判定顺序：交易所状态与生命周期 → 交易对全局准入 → COIN 默认与策略分类配置 → 最终有效交易池</span
        >
      </div>
      <div class="risk-layout">
        <section class="policy-panel data-card">
          <div class="data-card-heading">
            <div>
              <h2>分类准入策略</h2>
              <p>默认仅允许 COIN；父 Category 关闭会覆盖子分类，显式允许可覆盖默认关闭</p>
            </div>
          </div>
          ...
          <div class="policy-tree">
            <article v-for="parent in visibleTrees" :key="parent.category_key">
              <div class="policy-row parent" :class="{ blocked: !enabled(parent) }">
                <button class="tree-toggle" @click="toggleTree(parent.category_key)">
                  {{ expanded.has(parent.category_key) ? '−' : '+' }}</button
                ><span class="policy-name"
                  ><strong>{{ parent.name }}</strong
                  ><small>{{ parent.symbol_count }} 交易对 · {{ policyLabel(parent) }}</small></span
                ><a-tag :color="!enabled(parent) ? 'red' : admissions.has(parent.category_key) ? 'blue' : 'default'">{{
                  policyLabel(parent)
                }}</a-tag
                ><a-switch
                  :checked="enabled(parent)"
                  :checked-children="'允许'"
                  :un-checked-children="'关闭'"
                  @change="requestChange(parent, Boolean($event))"
                />
              </div>
              <div v-if="expanded.has(parent.category_key)">
                <div
                  ...
                  class="policy-row child"
                  :class="{ blocked: !enabled(parent) || !enabled(child) }"
                >
                  <span class="tree-branch">└</span
                  ><span class="policy-name"
                    ><strong>{{ child.name }}</strong
                    ><small
                      >{{ child.symbol_count }} 交易对 ·
                      {{ !enabled(parent) ? '父分类覆盖关闭' : policyLabel(child) }}</small
                    ></span
                  ><a-tag
                    :color="
                      !enabled(parent) || !enabled(child)
                        ? 'red'
                        : admissions.has(child.category_key)
                          ? 'blue'
                          : 'default'
                    "
                    >{{ !enabled(parent) ? '父级关闭' : policyLabel(child) }}</a-tag
                  ><a-switch
                    :checked="enabled(child)"
                    :disabled="!enabled(parent)"
                    @change="requestChange(child, Boolean($event))"
                  />
                </div>
              </div>
            </article>
        ...
        <section class="universe-panel data-card">
          <div class="data-card-heading">
            <div>
              <h2>生效预览与判定理由</h2>
              <p>直接展示后端 effective universe，并按当前状态逐页完整载入</p>
            </div>
            <span class="heading-meta">{{ visibleUniverse.length }} / {{ preview?.items.length ?? 0 }}</span>
          </div>
          <div class="universe-tools">
            <a-input
            ...
            ><a-radio-group v-model:value="universeMode" button-style="solid" size="small" @change="changeUniverseMode"
              ...
              ><a-radio-button value="excluded">排除</a-radio-button></a-radio-group
            >
          </div>
          <div class="universe-list">
            <div v-for="item in visibleUniverse" :key="item.symbol" class="universe-row">
              <span :class="item.effective ? 'effective' : 'excluded'"
                ><CheckCircle2 v-if="item.effective" :size="15" /><Ban v-else :size="15" /><strong>{{
                  item.symbol
                }}</strong></span
              >
              <div>
                <a-tag :color="item.effective ? 'green' : 'red'">{{ item.effective ? '有效' : '排除' }}</a-tag>
                <p>{{ reasons(item) }}</p>
                <small v-if="item.blocked_category_keys.length"
                  >阻断分类：{{
                    item.blocked_category_keys.map((key) => categoryByKey.get(key)?.name || key).join('、')
                  }}</small
                >
              </div>
            </div>
      ...
      <section class="data-card audit-panel">
        <div class="data-card-heading">
          <div>
            <h2>分类准入审计</h2>
            <p>版本冲突时重新读取，不覆盖他人变更</p>
          </div>
          <span class="heading-meta">{{ audits.length }} RECORDS</span>
        </div>
        <div v-if="audits.length" class="audit-scroll">
          <table class="audit-table">
            ...
            <tbody>
              <tr v-for="audit in audits" :key="audit.id">
                <td>{{ formatDateTime(audit.changed_at) }}</td>
                <td>{{ categoryByKey.get(audit.category_key)?.name || audit.category_key }}</td>
                <td>
                  {{ audit.previous_enabled == null ? '默认' : audit.previous_enabled ? '允许' : '关闭' }} →
                  {{ audit.enabled ? '允许' : '关闭' }}
                </td>
                <td>v{{ audit.version }} · {{ audit.changed_by }}</td>
                <td>{{ audit.reason || '—' }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        <a-empty v-else description="当前策略没有显式分类准入变更" />
      </section>
    </DataState>
    ...
    <a-modal
      ...
      :title="`${pendingEnabled ? '允许' : '关闭'} ${pendingCategory?.name || ''}`"
      ok-text="确认变更"
      :ok-button-props="{ disabled: !reason.trim(), danger: !pendingEnabled }"
      @ok="saveChange"
    >
      <a-alert
        v-if="!pendingEnabled"
        ...
        show-icon
        message="该分类将阻止策略新开仓"
        description="相关未成交入场单应由执行链路按已确认规则撤销；已有仓位继续保护与退出。页面只提交准入事实，不直接操作订单。"
      />
      <a-alert v-else type="info" show-icon message="允许分类不会绕过交易所状态或交易对全局禁用。" />
      <label class="reason-field"
        ><span>修改原因 *</span
        ><a-textarea v-model:value="reason" maxlength="500" show-count :rows="3" placeholder="说明本次策略分类准入变更"
      /></label>
    </a-modal>
  </main>
</template>
...
.strategy-selector label > span {
  color: var(--muted);
  font-size: var(--type-meta);
}
.strategy-selector > div {
  display: flex;
  align-items: center;
  gap: 6px;
...
.risk-layout {
  display: grid;
  grid-template-columns: minmax(440px, 1fr) minmax(390px, 1fr);
  gap: 12px;
  align-items: start;
}
.panel-search,
...
.policy-tree article:last-child {
  border-bottom: 0;
}
.policy-row {
  display: grid;
...
.policy-row.child {
  min-height: 47px;
  padding-left: 34px;
  background: rgba(80, 100, 120, 0.025);
}
.policy-row.blocked {
...
.universe-row > span.effective {
  color: var(--color-success);
}
.universe-row > span.excluded {
