...
const { loading, error, refreshedAt, reload } = useLedgerLoader(
  async ({ isStale }) => {
    ...
    const [
      ...
      accountId ? operationsApi.pnl({ account_id: accountId, ...query.value }) : Promise.resolve(null),
    ...
    if (
      [healthResult, runtimeResult, positionResult, activeOrdersResult, tradeResult, dailyResult].every(
...
<template>
  <main class="operations-page overview-page">
    ...
    >
      <template #actions>
        <a-tag v-for="mode in runtimeModes" :key="mode" :color="mode === 'live' ? 'red' : 'gold'">{{
          mode.toUpperCase()
        }}</a-tag>
    ...
    <DataState :loading="loading" :error="error" @retry="reload">
      <a-alert
        ...
        type="warning"
        show-icon
        :message="`部分数据读取失败：${unavailableSources.join('、')}`"
        description="失败项显示为“读取失败”，不会当作 0 或空数据。"
      ...
      <section class="health-rail" aria-label="运行健康状态">
        <article :class="['health-node', health?.status === 'healthy' ? 'ok' : 'bad']">
          <Database :size="18" />
          <div>
            <span>账本数据库</span><strong>{{ health?.status === 'healthy' ? '正常' : '不可用' }}</strong>
          </div>
        </article>
        <article :class="['health-node', unhealthyRuntimes.length ? 'bad' : runtimes.length ? 'ok' : 'unknown']">
          ...
          <div>
            <span>策略运行</span
            ><strong>{{
              runtimes.length ? `${runtimes.length - unhealthyRuntimes.length}/${runtimes.length} 正常` : '无心跳数据'
            }}</strong>
          </div>
        </article>
        <article class="health-node unknown">
          <RadioTower :size="18" />
          <div><span>行情连续性</span><strong>尚无独立查询接口</strong></div>
        </article>
        <article :class="['health-node', blockedRuntimes.length ? 'bad' : runtimes.length ? 'ok' : 'unknown']">
          ...
          <div>
            ...
            ><strong>{{
              runtimes.length
                ? blockedRuntimes.length
                  ? `${blockedRuntimes.length} 个受限`
                  : '允许入场'
                : '无运行数据'
            }}</strong>
          </div>
      ...
      <section class="metric-grid overview-metrics">
        ...
        <MetricTile
          label="当前持仓"
          :value="positionsTotal == null ? '读取失败' : String(positionsTotal)"
          hint="进入持仓明细"
          :tone="positionsTotal == null ? 'warning' : 'neutral'"
          :to="{ path: '/positions', query: { ...query, tab: 'positions' } }"
        />
        <MetricTile
          label="活动订单"
          :value="activeOrdersTotal == null ? '读取失败' : String(activeOrdersTotal)"
          hint="NEW + PARTIALLY_FILLED"
          :tone="activeOrdersTotal == null || activeOrdersTotal > 0 ? 'warning' : 'neutral'"
          :to="{ path: '/positions', query: { ...query, tab: 'active' } }"
        />
      </section>
      ...
      <section class="capital-card" aria-label="策略资金状态">
        <header class="capital-heading">
          <div class="capital-title">
            ...
            <div>
              <h2>策略资金状态</h2>
              <p v-if="capital">
                <span>{{ capital.account_id }}</span> / <span>{{ capital.strategy_id }}</span>
              </p>
        ...
        <div v-if="capital" class="capital-grid">
          <div class="capital-primary">
            <span>可交易资金</span><strong>{{ formatMoney(capital.trading_capital) }} <small>USDT</small></strong>
          </div>
          <div>
            <span>账户资金</span><strong>{{ formatMoney(capital.account_capital) }} <small>USDT</small></strong>
          </div>
          <div>
            <span>储备资金</span><strong>{{ formatMoney(capital.reserve_capital) }} <small>USDT</small></strong>
          </div>
          <div>
            <span>停止开仓阈值</span><strong>{{ formatMoney(capital.minimum) }} <small>USDT</small></strong>
          </div>
          <div>
            <span>盈利复投比例</span><strong>{{ formatPercent(capital.profit_reinvest_ratio, 0) }}</strong>
          </div>
          <div>
            <span>资金越界</span
            ><strong :class="capital.capital_breached ? 'value-negative' : 'value-positive'">{{
              capital.capital_breached ? '是' : '否'
            }}</strong>
          </div>
          <div>
            <span>状态版本</span><strong>v{{ capital.version }}</strong>
          </div>
          <div>
            <span>更新时间</span><strong class="capital-time">{{ formatDateTime(capital.updated_at) }}</strong>
          </div>
        </div>
      ...
      <section class="two-column overview-panels">
        <article class="data-card calendar-card">
          <div class="data-card-heading">
            <div>
              <h2><CalendarDays :size="15" /> 收益日历</h2>
              <p>{{ year }} 年 {{ month }} 月 · {{ closedPnlScope }} · 日界线 Asia/Shanghai</p>
            </div>
          ...
          <div v-if="!daily.length" class="inline-empty">
            {{ unavailableSources.includes('当月已实现收益') ? '收益日历读取失败' : '本月没有已实现收益记录' }}
          </div>
        </article>
        ...
        <article class="data-card recent-card">
          <div class="data-card-heading">
            ...
            <RouterLink :to="{ path: '/trades', query: query }" class="table-link">全部成交 →</RouterLink>
          </div>
          <div v-if="recentTrades.length" class="recent-list">
            <button v-for="trade in recentTrades" :key="trade.id" type="button" @click="openRecentTrade(trade)">
              ...
              <div>
                <strong :class="pnlClass(trade.realized_pnl)">{{ formatMoney(trade.realized_pnl) }}</strong
                ><time>{{ formatDateTime(trade.exchange_time) }}</time>
              </div>
            </button>
          </div>
...
.capital-title p {
  margin: 3px 0 0;
  color: var(--muted);
  font: var(--type-meta)/1.35 var(--font-family-mono);
...
.capital-grid > div:nth-child(4n) {
  border-right: 0;
}
.capital-grid > div:nth-last-child(-n + 4) {
  border-bottom: 0;
}
.capital-grid > .capital-primary {
...
.capital-grid strong {
  overflow-wrap: anywhere;
  font: 600 var(--type-primary)/1.3 var(--font-family-mono);
}
.capital-grid small {
  color: var(--muted);
  font-size: var(--type-meta);
...
.recent-list button:last-child {
  border-bottom: 0;
}
.recent-list button:hover {
  background: var(--surface-hover);
}
.recent-list button > div:last-child {
  text-align: right;
}
.recent-list strong,
.recent-list span,
...
@media (max-width: 1000px) {
  ...
  .capital-grid > div:nth-child(2n) {
    border-right: 0;
  }
  .capital-grid > div:nth-last-child(-n + 2) {
    border-bottom: 0;
  }
}
