...
// 分组维度原先只存在于 URL 里，而 syncRoute 是整体替换 query——
...
<template>
  <main class="operations-page performance-page">
    <PageHeader
      eyebrow="ANALYSIS / CAMPAIGN METRICS"
      title="绩效分析"
      description="统计单位固定为完整且已结束的 Campaign；胜率和盈亏比用于诊断，不作为策略自动启停门槛。"
      :loading="loading"
    ...
    <div class="date-filter">
      <label
        ><span>开始日期</span
        ><a-date-picker :value="startDate" value-format="YYYY-MM-DD" @update:value="startDate = String($event ?? '')"
      /></label>
      <span class="date-rule">ASIA/SHANGHAI</span>
      <label
        ><span>结束日期</span
        ><a-date-picker :value="endDate" value-format="YYYY-MM-DD" @update:value="endDate = String($event ?? '')"
      /></label>
    ...
    <a-alert
      ...
      show-icon
      message="请选择账户 ID"
      description="绩效分析按账户归属；选择账户后显示完整 Campaign 指标。"
    />
    ...
    <template v-else>
      ...
      <DataState :loading="loading" :error="error" :empty="!summary" @retry="reload">
        <template v-if="summary">
          <div class="metric-scope">
            <Info :size="14" /><span
              >样本口径：<strong>{{ scopeText }}</strong></span
            ><span>时间：{{ startDate }} → {{ endDate }}</span
            ><span>样本：{{ sampleSize }} 轮次 / {{ summary.total_fills }} fills</span
            ><span>成本：净 PnL 仅扣 USDT 手续费；资金费不可用</span>
          </div>
          <a-alert
            v-if="sampleSize < 30"
            type="warning"
            show-icon
            :message="`样本不足：当前仅 ${sampleSize} 个已结束轮次`"
            description="指标仍按真实样本展示，但不建议据此自动改变策略准入。"
          ...
          <template v-if="activeTab === 'overview'">
            <section class="metric-grid">
              ...
              <MetricTile
                label="纳入轮次"
                :value="String(summary.candidate_campaigns - summary.excluded_campaigns)"
                :hint="`候选 ${summary.candidate_campaigns} / 排除 ${summary.excluded_campaigns}`"
              />
              <MetricTile
                label="未归属 fills"
                :value="String(summary.unattributed_fills)"
                :tone="summary.unattributed_fills ? 'warning' : 'neutral'"
                hint="不计入 Campaign 指标"
              />
            </section>
            <section class="data-card daily-chart-card">
              <div class="data-card-heading">
                <div>
                  <h2>每日已实现净 PnL</h2>
                  <p>日界线 Asia/Shanghai；缺失日期不插值</p>
                </div>
                <span class="heading-meta">{{ daily.length }} DATA POINTS</span>
              </div>
          ...
          <template v-else-if="activeTab === 'quality'">
            <section class="quality-hero">
              <article>
                <span>胜率</span><strong>{{ formatPercent(summary.win_rate) }}</strong>
                <p>{{ summary.win_count }} 盈 / {{ summary.loss_count }} 亏 / {{ summary.flat_count }} 平</p>
              </article>
              <div class="quality-cross"><Scale :size="24" /><span>必须与样本和收益尺度一起阅读</span></div>
              <article>
                <span>盈亏比</span><strong>{{ formatRatio(summary.payoff_ratio) }}</strong>
                <p>平均盈利 ÷ 平均亏损绝对值</p>
              </article>
            </section>
            <section class="metric-grid quality-metrics">
              <MetricTile
                label="平均盈利"
                :value="formatMoney(summary.avg_win)"
                tone="positive"
                :hint="`${summary.win_count} 个盈利轮次`"
              />
              <MetricTile
                label="平均亏损"
                :value="formatMoney(summary.avg_loss)"
                tone="negative"
                :hint="`${summary.loss_count} 个亏损轮次`"
              />
              <MetricTile
                label="单轮期望值"
            ...
            <a-alert
              v-if="summary.loss_count === 0"
              type="info"
              show-icon
              message="没有亏损样本，盈亏比和 Profit Factor 显示为 —，不会显示无穷大。"
            />
            <a-alert
              type="info"
              show-icon
              message="部分诊断指标暂不可用"
              description="当前账本没有持仓时长、盈亏分布和权益时序接口；页面不会用 fills 推测这些指标。"
            />
          </template>
          ...
          <template v-else>
            ...
            <template v-else-if="breakdown">
              ...
              <div class="table-frame breakdown-table">
                <a-table
                  :data-source="breakdown.items"
                  row-key="dimension_key"
                  :pagination="false"
                  :scroll="{ x: 1100 }"
                  size="middle"
                >
                  <a-table-column key="dimension" title="维度" data-index="dimension_label"
                    ><template #default="{ record }"
                      ><strong>{{ record.dimension_label || '未分类' }}</strong
                      ><small class="table-subtext">{{
                        record.dimension_key || 'NO ACTIVE ASSOCIATION'
                      }}</small></template
                    ></a-table-column
                  >
                  <a-table-column key="trades" title="轮次" data-index="total_trades" />
                  <a-table-column key="win_rate" title="胜率"
                    ><template #default="{ record }">{{ formatPercent(record.win_rate) }}</template></a-table-column
                  >
                  <a-table-column key="payoff" title="盈亏比"
                    ><template #default="{ record }">{{ formatRatio(record.payoff_ratio) }}</template></a-table-column
                  >
                  <a-table-column key="net" title="净 PnL"
                    ><template #default="{ record }"
                      ><span :class="pnlClass(record.net_pnl)">{{ formatMoney(record.net_pnl) }}</span></template
                    ></a-table-column
                  >
                  <a-table-column key="drawdown" title="最大回撤"
                    ><template #default="{ record }">{{ formatMoney(record.max_drawdown) }}</template></a-table-column
                  >
                </a-table>
              </div>
            </template>
            <section v-else class="breakdown-panel">
              <BarChart3 :size="30" />
              <div>
                <h2>读取绩效分组</h2>
                <p>按交易对、Category、Subcategory 或方向查看完整 Campaign 聚合。</p>
              </div>
            </section>
          </template>
        </template>
...
.bar-track i.value-positive {
  background: var(--color-success);
}
.bar-track i.value-negative {
...
.quality-hero strong {
  display: block;
  margin: 12px 0 8px;
  font: 600 36px var(--font-family-mono);
}
.quality-hero p {
  margin: 0;
  color: var(--muted);
