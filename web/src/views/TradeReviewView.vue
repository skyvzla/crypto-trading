...
const {
  ...
  restore: restorePage,
  apply: applyPage,
} = usePageParams({ defaultSize: 50, maxSize: 1000 })
...
const campaignColumns: TableColumnsType<CampaignSummary> = [
  {
    ...
    fixed: 'left',
    width: 132,
    customRender: ({ text }) => h('strong', { class: 'campaign-symbol' }, String(text)),
  },
  {
    title: 'Campaign',
    ...
    customRender: ({ record }) =>
      h(
        Button,
        { type: 'link', class: 'campaign-button', onClick: () => openCampaign(record) },
        () => record.campaign_id,
      ),
  },
  {
    ...
    customRender: ({ record }) =>
      h(
        Tag,
        {
          ...
          color: record.closed_at ? 'success' : record.has_open_quantity ? 'processing' : 'warning',
        },
        () => (record.closed_at ? '已结束' : record.has_open_quantity ? '进行中' : '事实不完整'),
      ),
  },
  { title: 'fills', dataIndex: 'fill_count', key: 'fills', width: 75 },
  {
    title: '净已实现 PnL',
    dataIndex: 'net_realized_pnl',
    key: 'netPnl',
    width: 135,
    customCell: (record) => ({
      class: record.net_realized_pnl == null ? 'value-unavailable' : pnlClass(record.net_realized_pnl),
    }),
    customRender: ({ record }) =>
      record.net_realized_pnl == null
        ? h(
            'span',
            {
              class: 'net-unavailable',
              title:
                record.commission_asset === 'USDT' ? 'Campaign 尚未闭合或 PnL 事实不完整' : '手续费缺少权威 USDT 换算',
            },
            '不可用',
          )
        : formatMoney(record.net_realized_pnl),
  },
  {
    title: '毛 PnL',
    dataIndex: 'gross_realized_pnl',
    key: 'grossPnl',
    width: 115,
    customRender: ({ text }) => formatMoney(String(text)),
  },
  {
    title: '手续费',
    key: 'commission',
    width: 150,
    customRender: ({ record }) => `${formatMoney(record.total_commission)} ${record.commission_asset || '资产不一致'}`,
  },
  { title: '账户', dataIndex: 'account_id', key: 'account', width: 145 },
  { title: '策略', dataIndex: 'strategy_id', key: 'strategy', width: 145 },
  {
    title: '首笔成交',
    dataIndex: 'first_fill_at',
    key: 'first',
    width: 185,
  ...
  },
  {
    ...
    key: 'closed',
    width: 185,
    customRender: ({ record }) => formatDateTime(record.closed_at || record.last_fill_at),
  },
]
...
const { loading, error, refreshedAt, reload } = useLedgerLoader(
  async ({ isStale }) => {
    const result = await operationsApi.campaigns({
      ...
      ...(requestedCampaign.value ? { campaign_id: requestedCampaign.value } : {}),
      ...(selectedDate.value
        ? { start_date: selectedDate.value, end_date: selectedDate.value, timezone: LEDGER_TIMEZONE }
        : {}),
...
<template>
  <main class="operations-page trade-review-page">
    <PageHeader
      ...
      title="成交复盘"
      description="以服务端完整 Campaign 为浏览单位，下钻到全部账本 fills 和策略审计事件。"
      :loading="loading"
      ...
      <template #extra-fields>
        <label class="trade-date-filter"
          ...
          ><a-date-picker
            ...
            allow-clear
            placeholder="全部日期"
            @update:value="selectedDate = String($event ?? '')"
        /></label>
      </template>
    </FilterBar>
    <div class="review-tools">
      <span
        >服务端共 {{ serverTotal }} Campaigns · 当前页 {{ campaigns.length }} 个 · 未归属
        {{ unattributedFills }} fills</span
      >
    </div>
    <a-alert
      v-if="unattributedFills"
      type="warning"
      show-icon
      :message="`${unattributedFills} 笔成交缺少 Campaign 归属`"
      description="未归属 fills 不参与 Campaign 列表、收益日历或绩效统计，页面不会按时间相邻关系猜测归属。"
      class="scope-alert"
    />
    <a-alert
      v-else
      type="info"
      show-icon
      message="Campaign 由服务端完整聚合"
      description="分页单位为完整 Campaign；非 USDT 手续费没有权威换算时，净 PnL 明确显示不可用。结果与规范化退出原因筛选仍待权威字段。"
      class="scope-alert"
    />
