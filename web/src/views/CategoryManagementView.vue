...
const orphans = computed(() =>
  categories.value.filter(
    (item) =>
      item.category_type === 'SUBCATEGORY' &&
      !categories.value.some((parent) => parent.category_key === item.parent_key),
  ),
)
const visibleParents = computed(() => {
  ...
  return parents.value.filter((parent) =>
    [parent.name, parent.code, ...parent.children.flatMap((child) => [child.name, child.code])].some((value) =>
...
function routeQuery() {
  ...
  return {
    q: search.value.trim(),
    ...(viewingUnclassified.value
      ? { unclassified: 'true' }
      : selected.value
        ? { category: selected.value.category_key }
        : {}),
...
<template>
  <main class="operations-page categories-page">
    <PageHeader
      eyebrow="REFERENCE DATA / BINANCE TAXONOMY"
      title="分类管理"
      description="只读展示程序同步的 Category → Subcategory → 交易对关系；Web 不创建、重命名或编辑同步事实。"
      :loading="loading"
    ...
    <div
      v-if="syncStatus"
      :class="['status-strip', { stale: syncStatus.stale, error: syncStatus.status !== 'SUCCESS' }]"
    >
      <DatabaseBackup :size="14" /><span
        >来源 <strong>Binance 同步事实</strong> · {{ syncStatus.status }} · 最近成功
        {{ formatDateTime(syncStatus.last_success_at) }} · 允许最大年龄 {{ syncStatus.max_age_hours }}h</span
      ><a-tag v-if="syncStatus.stale" color="gold">STALE · 历史数据仍展示</a-tag>
    </div>
    <div class="category-tools">
      ...
      ><a-button :type="viewingUnclassified ? 'primary' : 'default'" @click="selectUnclassified()"
        ><template #icon><ListFilter :size="14" /></template>未分类交易对</a-button
      ><span>{{ parents.length }} Category · {{ categories.length - parents.length }} Subcategory</span>
    </div>
    <a-alert
      v-if="syncStatus?.last_error"
      type="warning"
      show-icon
      :message="syncStatus.last_error"
      description="保留并展示最近一次成功同步的数据。"
      class="sync-error"
    />
      ...
      <div class="category-layout">
        <section class="taxonomy-tree">
          <article v-for="parent in visibleParents" :key="parent.category_key" class="taxonomy-parent">
            <div
              class="taxonomy-node parent"
              :class="{ selected: selected?.category_key === parent.category_key, inactive: !parent.active }"
            >
              <button
                type="button"
                class="expand-control"
                :aria-label="expanded.has(parent.category_key) ? '收起子分类' : '展开子分类'"
                :aria-expanded="expanded.has(parent.category_key)"
                @click="toggle(parent.category_key)"
              >
                <ChevronDown v-if="expanded.has(parent.category_key)" :size="14" /><ChevronRight v-else :size="14" />
              </button>
              <button type="button" class="parent-select" @click="selectCategory(parent)">
                <FolderTree :size="15" /><span class="node-copy"
                  ><strong>{{ parent.name }}</strong
                  ><small>{{ parent.code }} · {{ parent.source }}</small></span
                ><a-tag>{{ parent.symbol_count }} 交易对</a-tag
                ><a-tag v-if="!parent.active" color="default">INACTIVE</a-tag>
              </button>
            </div>
            <div v-if="expanded.has(parent.category_key) || search.trim()" class="taxonomy-children">
              <button
                ...
                :class="{ selected: selected?.category_key === child.category_key, inactive: !child.active }"
                @click="selectCategory(child)"
              >
                <span class="tree-line">└</span
                ><span class="node-copy"
                  ><strong>{{ child.name }}</strong
                  ><small>{{ child.code }}</small></span
                ><a-tag>{{ child.symbol_count }}</a-tag
                ><a-tag v-if="!child.active">INACTIVE</a-tag>
              </button>
              <div v-if="!parent.children.length" class="no-children">没有 Subcategory</div>
            </div>
          </article>
          <article v-if="orphans.length" class="orphan-block">
            <h3>缺少父节点的 Subcategory</h3>
            <button
            ...
            >
              ...
              ><span class="node-copy"
                ...
                ><small>{{ item.parent_key || 'NO PARENT KEY' }}</small></span
              ><a-tag color="gold">{{ item.symbol_count }}</a-tag>
            </button>
          </article>
        ...
        <aside class="category-detail data-card">
          <template v-if="selected || viewingUnclassified">
            <div class="data-card-heading">
              <div>
                <h2>{{ viewingUnclassified ? '未分类交易对' : selected?.name }}</h2>
                <p>
                  {{
                    viewingUnclassified
                      ? 'NO ACTIVE CATEGORY ASSOCIATION'
                      : `${selected?.category_type} · ${selected?.category_key}`
                  }}
                </p>
              </div>
              <span class="heading-meta">{{ selectedTotal }} SYMBOLS</span>
            </div>
            <a-spin :spinning="detailLoading">
              <div v-if="selectedSymbols.length" class="category-symbol-list">
                <div v-for="symbol in selectedSymbols" :key="symbol.symbol" class="category-symbol-row">
                  <span
                    ><strong>{{ symbol.symbol }}</strong
                    ><small>{{ symbol.base_asset }} / {{ symbol.quote_asset }}</small></span
                  ><span
                    ><a-tag :color="symbol.status === 'TRADING' ? 'green' : 'gold'">{{ symbol.status }}</a-tag
                    ><a-tag :color="symbol.global_enabled ? 'blue' : 'red'">{{
                      symbol.global_enabled ? '全局允许' : '全局禁止'
                    }}</a-tag></span
                  >
                </div>
              </div>
              <a-empty
                ...
                :description="viewingUnclassified ? '当前没有未分类交易对' : '该分类没有关联交易对'"
              />
              <div v-if="selectedTotal > DETAIL_PAGE_SIZE" class="detail-pagination">
                <a-pagination
                  :current="detailPage"
          ...
          <div v-else class="select-hint">
            <FolderTree :size="28" />
            <p>选择左侧 Category 或 Subcategory 查看关联交易对和当前有效状态。</p>
          </div>
        </aside>
      </div>
    </DataState>
...
.category-layout {
  display: grid;
  grid-template-columns: minmax(360px, 0.85fr) minmax(390px, 1.15fr);
  gap: 12px;
  align-items: start;
}
.taxonomy-tree {
...
.taxonomy-node.child:last-child {
  border-bottom: 0;
}
.tree-line {
  color: var(--color-gold);
...
.category-symbol-list button:last-child {
  border-bottom: 0;
}
.category-symbol-list strong,
.category-symbol-list small {
  display: block;
}
.category-symbol-list strong {
...
.category-symbol-row:last-child {
  border-bottom: 0;
}
.detail-pagination {
  display: flex;
