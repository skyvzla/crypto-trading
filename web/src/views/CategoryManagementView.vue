        ...
        <section class="taxonomy-tree">
          <article v-for="parent in visibleParents" :key="parent.category_key" class="taxonomy-parent">
            <button type="button" class="taxonomy-node parent" :class="{ selected: selected?.category_key === parent.category_key, inactive: !parent.active }" @click="selectCategory(parent)">
              <span class="expand-control" @click.stop="toggle(parent.category_key)">{{ expanded.has(parent.category_key) ? '−' : '+' }}</span><FolderTree :size="15" /><span class="node-copy"><strong>{{ parent.name }}</strong><small>{{ parent.code }} · {{ parent.source }}</small></span><a-tag>{{ parent.symbol_count }} 交易对</a-tag><a-tag v-if="!parent.active" color="default">INACTIVE</a-tag>
            </button>
            <div v-if="expanded.has(parent.category_key)" class="taxonomy-children">
        ...
        <aside class="category-detail data-card">
          <template v-if="selected || viewingUnclassified">
            ...
            <a-spin :spinning="detailLoading">
              <div v-if="selectedSymbols.length" class="category-symbol-list"><button v-for="symbol in selectedSymbols" :key="symbol.symbol" type="button"><span><strong>{{ symbol.symbol }}</strong><small>{{ symbol.base_asset }} / {{ symbol.quote_asset }}</small></span><span><a-tag :color="symbol.status === 'TRADING' ? 'green' : 'gold'">{{ symbol.status }}</a-tag><a-tag :color="symbol.global_enabled ? 'blue' : 'red'">{{ symbol.global_enabled ? '全局允许' : '全局禁止' }}</a-tag></span></button></div>
