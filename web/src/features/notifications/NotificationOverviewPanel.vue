...
<template>
  <section class="view-panel overview-view" aria-labelledby="overview-heading">
    <NotificationSectionHeader
      ...
      kicker="CONTROL ROOM"
      title="概览"
      note="配置变更即时生效，历史投递保留原始快照"
    />

    <div class="metric-grid notification-metrics">
      <MetricTile
        label="启用连接器"
        :value="`${overview.enabled_connectors}/${overview.connectors}`"
      ...
      <MetricTile
        ...
        :hint="deadDeliveryCount ? `${deadDeliveryCount} 条死信` : '当前无死信'"
        :tone="retryDeliveryCount > 0 ? 'warning' : 'neutral'"
      />
    </div>
    ...
    <div class="overview-grid">
      <section class="data-card health-panel">
        <div class="data-card-heading">
          <div>
            <CircleDot :size="16" class="panel-icon" />
            <h3>投递状态</h3>
          </div>
        ...
        <a-button type="link" class="panel-link" @click="emit('open-activity', 'deliveries')"
          >查看投递队列 <ArrowRight :size="14"
        /></a-button>
      </section>
      <section class="data-card quick-panel">
        <div class="data-card-heading">
          <div>
            <SlidersHorizontal :size="16" class="panel-icon" />
            <h3>快速配置</h3>
          </div>
          <span class="quiet-value">常用动作</span>
        </div>
        <button type="button" class="quick-action" @click="emit('new-connector', 'telegram')">
          ...
          ><span class="quick-arrow"><Plus :size="16" /></span>
        </button>
        <button type="button" class="quick-action" @click="emit('new-connector', 'webhook')">
          <span class="quick-mark webhook"><Globe2 :size="15" /></span
          ><span><strong>添加 Webhook</strong><small>同一鉴权配置可挂多个 URL</small></span
          ><span class="quick-arrow"><Plus :size="16" /></span>
        </button>
        <button type="button" class="quick-action" @click="emit('new-policy')">
    ...
    <section class="data-card recent-panel">
      <div class="data-card-heading">
        <div>
          <Activity :size="16" class="panel-icon" />
          <h3>最近事件</h3>
        </div>
        <a-button type="link" class="panel-link" @click="emit('open-activity', 'events')"
      ...
      <div v-if="events.items.length" class="event-list">
        <div v-for="event in events.items.slice(0, 5)" :key="event.id" class="event-row">
          ...
          <div class="event-copy">
            ...
            ><small>{{ event.event_type }} · {{ event.source }}</small>
          </div>
          <a-badge :status="statusBadge(event.routing_status)" :text="statusLabel(event.routing_status)" /><time>{{
            formatShortTime(event.occurred_at)
          }}</time>
        </div>
      </div>
...
.health-list > div:nth-last-child(-n + 2) {
  border-bottom: 0;
}
.health-list strong {
  color: var(--text);
...
:root[data-theme='dark'] .quick-mark.webhook {
  color: #5eead4;
}
.quick-arrow {
  display: inline-flex;
...
.event-row {
  display: grid;
  grid-template-columns: 12px minmax(0, 1fr) auto 128px;
  align-items: center;
  gap: 9px;
  min-height: 47px;
  border-bottom: 1px solid color-mix(in srgb, var(--muted) 15%, transparent);
