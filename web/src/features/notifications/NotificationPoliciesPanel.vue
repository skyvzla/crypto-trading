...
<template>
  <section class="view-panel" aria-labelledby="policies-heading">
    <NotificationSectionHeader
      ...
      title="路由策略"
      description="精确事件模式优先，其次按优先级选择；一个策略可通知一个或多个职责组，也可以显式抑制。"
    >
      <template #actions
        ><a-button type="primary" @click="emit('new')"
          ><template #icon><Plus :size="15" /></template>新建策略</a-button
        ></template
      >
    </NotificationSectionHeader>
    <div class="data-table-wrap data-card">
      <a-table :data-source="policies" :pagination="false" :scroll="{ x: 700 }" row-key="id" size="small">
        <a-table-column key="name" title="策略" :width="190"
          ><template #default="{ record }"
            ><div class="primary-cell">
              <strong>{{ record.name }}</strong
              ><small>v{{ record.version }} · {{ record.suppress ? '显式抑制' : '正常路由' }}</small>
            </div></template
          ></a-table-column
        >
        <a-table-column key="pattern" title="事件模式"
          ><template #default="{ record }"
            ><code class="pattern-code">{{ record.event_pattern }}</code></template
          ></a-table-column
        >
        <a-table-column key="severity" title="级别" :width="92"
          ><template #default="{ record }"
            ><a-badge :status="statusBadge(record.severity)" :text="severityLabel(record.severity)" /></template
        ></a-table-column>
        <a-table-column key="priority" title="优先级" :width="76"
          ><template #default="{ record }"
            ><span class="mono-value">{{ record.priority }}</span></template
          ></a-table-column
        >
        <a-table-column key="groups" title="职责组"
          ><template #default="{ record }"
            ><span class="wrap-value">{{
              record.suppress ? '—' : nameList(record.group_ids, props.groupById, '未选择职责组')
            }}</span></template
          ></a-table-column
        >
        ...
        <a-table-column key="actions" title="操作" :width="116"
          ><template #default="{ record }"
            ><a-button type="text" aria-label="编辑路由策略" @click="emit('edit', record)"
            ...
            ><a-popconfirm
              ...
              ><a-button type="text" danger aria-label="删除路由策略"
