...
- 已建立 Git 仓库并提交初始版本；
- 已确认三层业务架构；
- 本地上次全量测试为 `159 passed, 9 skipped`，当前 Compose 真实 Redis/PostgreSQL 环境为 `174 passed`；
...
| User Stream 与启动对账 | 部分完成 | 具体账户进程、成交事实对账、断线补偿 | 已接好 WAL/风险门禁/账本回调、启动未知单解析、订单/仓位快照一致性门禁、后台轮询和停止/重连；轮询参数及账户进程待确认 |
...
当前基线：本地上次 `159 passed, 9 skipped`；Compose 真实 Redis/PostgreSQL 环境 `174 passed`。
