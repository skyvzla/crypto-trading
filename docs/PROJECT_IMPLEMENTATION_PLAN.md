...
- 已建立 Git 仓库并提交初始版本；
- 已确认三层业务架构；
- 本地上次全量测试为 `159 passed, 9 skipped`，当前 Compose 真实 Redis/PostgreSQL 环境为 `175 passed`；
...
| 全局交易准入与首成交计时 | 部分完成 | Campaign 持久化、恢复、终态 | 已验证全局互斥、首成交计时及 D-009“盈利超过 900 秒时先平旧轮次再激活新信号” |
| 入场幂等与失效撤单 | 部分完成 | 后台恢复、撤单竞态、迟到回报 | replay 已验证幂等和失效撤单；订单 WAL 已接入 Binance REST 提交适配器 |
| User Stream 与启动对账 | 部分完成 | 具体账户进程、成交事实对账 | 已接好 WAL/风险门禁/账本回调、启动未知单解析、启动及重连后的订单/仓位快照一致性门禁、后台轮询和停止/重连；轮询参数及账户进程待确认 |
...
| subcategory 准入控制 | 部分完成 | 接入具体账户进程、接入可交易池扫描器并外部验证 | 乐观并发、追加审计、扫描驱动的 fail-closed 刷新、全局门禁和关闭撤单已通过真实 PostgreSQL 组合测试 |
...
次数和账户归属，建立具体 testnet 进程，补成交事实启动对账、撤单竞态、
迟到回报、保护单及外部测试网验证。
...
当前基线：本地上次 `159 passed, 9 skipped`；Compose 真实 Redis/PostgreSQL 环境 `175 passed`。
...
| Q-003 | 已确认起涨点为第一检查位、动能衰减时减半；仍需确认动能定义、剩余半仓退出，以及前 90 秒/其后的风险保护 | 建议先用 2026 年 7 月 AKEUSDT replay 标定候选阈值，再由用户冻结 |
...
| Q-007 | 监听租约的入池、回吐、期限、扫描周期和重入规则 | subcategory 已确认共用该扫描周期；建议用状态机和显式 TTL，参数由业务确认 |
