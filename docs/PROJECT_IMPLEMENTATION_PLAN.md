...
| 入场幂等与失效撤单 | 部分完成 | 完整策略进程及部分成交外部验收 | replay 已验证幂等和失效撤单；AKEUSDT testnet 已真实完成预挂撤单和限价成交；WAL、撤单锁、迟到回报及未知状态已有自动化覆盖 |
| User Stream 与启动对账 | 部分完成 | 带真实订单的 one-way testnet 外部竞态验证 | 已接好具体账户进程、WAL/风险门禁/账本回调、REST 成交回补、启动及重连严格对账和 5秒×12 次未知单轮询；空仓启动/人工重启已验收，成交后等待仓位流确认 |
...
现有 one-way testnet 账户已补预挂撤单、成交、reduce-only 退出、紧急清仓及完整进程空仓
启动/人工重启；剩余带真实订单的策略进程、部分成交、保护退出和异常断线恢复外部验证。
