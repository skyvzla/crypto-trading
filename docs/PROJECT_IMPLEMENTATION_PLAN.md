...
- 已建立 Git 仓库并提交初始版本；
- 已确认三层业务架构；
- 当前 Compose 真实 Redis/PostgreSQL 全量测试为 `179 passed, 1 warning`；
...
- Redis 全局 Campaign 互斥存储组件已完成：原子获取、不自动过期、仅持有者可释放；
  当前尚未接入具体策略进程，释放前必须以交易所订单和仓位均终态为准。
...
当前 Compose 真实 Redis/PostgreSQL 全量测试基线为 `179 passed, 1 warning`。
...
| 编号 | 需要确认 | 建议 |
|---|---|---|
| Q-001 | Redis Campaign 从活跃恢复到终态时，交易所事实与本地账本的完整判定顺序 | 建议订单全部终态且仓位归零后才释放，全程 fail-closed |
| Q-002 | 三档 SELL limit 的“仅最高档成交”与价格穿透矛盾如何解释 | 建议先确认档位命名或是否分阶段挂单 |
| Q-003 | 已确认起涨点为第一检查位、动能衰减时减半；仍需确认专业动能指标、时间收严曲线和剩余半仓趋势退出 | 建议先用历史样本标定候选阈值，再由用户冻结 |
