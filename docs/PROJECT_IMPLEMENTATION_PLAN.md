...
| User Stream 与启动对账 | 部分完成 | one-way testnet 外部竞态验证 | 已接好具体账户进程、WAL/风险门禁/账本回调、REST 成交回补、启动及重连严格对账和 5秒×12 次未知单轮询；成交后等待仓位流确认 |
...
现有 one-way testnet 账户已补预挂撤单、成交、reduce-only 退出及紧急清仓；剩余完整策略进程、
User Stream、部分成交、保护退出和断线恢复的外部多轮验证。
