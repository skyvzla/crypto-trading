...
| 可交易池扫描编排 | 部分完成 | 完善发现入池和监听租约规则 | 固定每 5 分钟扫描；exchangeInfo 每日落库并按 15 天窗口做退市硬门禁，subcategory 同节拍刷新 |
...
| Q-008 | Web 敏感操作范围（认证与角色部分已由 D-029 关闭：不实现） | 建议 V1 最小只读 + subcategory 开关并保留审计 |
