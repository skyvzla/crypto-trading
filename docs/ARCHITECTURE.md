...
负责 Spike 策略的统一核心、逐币种 Campaign 状态、信号计算、订单意图、账户/币种风控、Binance REST/User Stream、订单幂等、WAL、对账、保护和退出。

`replay`、`testnet`、`live` 是此层的运行适配模式。回测是离线适配，不另设第四个业务层。执行客户端是库，不通过独立交易代理服务转发。

## 3. 账本控制层

负责 PostgreSQL 中的订单、成交、持仓、Campaign、控制状态、配置和审计记录；通过 FastAPI 提供查询和控制接口；Web 提供交易池 subcategory 控制、账本查看、盈亏和运行状态。
...
- 账本控制层不替代交易所成交事实，不绕过 Campaign 和风控。
