...
| Web | 运行总览、收益日历、持仓与订单、成交复盘、Campaign 绩效、回测复盘、策略分类风控、交易对管理和只读分类管理 | 应用内认证与角色不实现；资金费、权益时序、持仓时长和实盘 Campaign K 线因权威数据缺失而明确不可用 |
...
| **Web** | ✅ 运行总览、独立收益日历、持仓与订单、成交复盘、Campaign 绩效、回测复盘、策略分类风控、交易对管理和只读分类管理；✅ 账本健康与策略状态分离；✅ 应用内认证和角色不实现；⏳ 权威资金费、权益时序、持仓时长、实盘 K 线及敏感操作范围 | Phase 4 |
...
- 外部告警通道、Web 敏感操作范围、正式 live 风控/验收阈值及自然策略信号退出验收
...
6. Web V1 的敏感操作范围是什么？（身份认证与角色已确认不实现，见 D-029）
...
| Web 运行闭环 | 完成首期范围 | 已完成确认的信息架构和 Campaign 绩效；应用内认证和角色不实现，仍缺敏感操作范围确认、浏览器视觉验收及资金费等权威数据 |

**Phase 0 剩余工作**：
- 固定案例已完成 5/5；仍需结合逐笔数据解释对照脚本 CSV 差异，策略参数继续冻结

**Phase 1 剩余项**：
- Binance testnet 公共流、确定性回补、进程重启订阅恢复和 3900 秒长稳均已验证
- 外部告警通道
- 待确认的监听租约规则
