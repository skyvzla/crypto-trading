...
- WAL 已接入 Binance REST 提交适配器：提交前记录意图，网络状态不明时保持 `SUBMIT_UNKNOWN`，重复 client ID 不重下单；已有启动时逐订单单次查单入口，仍需后台轮询编排和账户级风险门禁
...
| **执行恢复** | ✅ WAL/REST 提交/启动单次 `SUBMIT_UNKNOWN` 解析；⏳后台轮询、完整启动对账、迟到回报 | Phase 3 |
