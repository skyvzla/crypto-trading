...
当前本地全量测试为 `39 passed`。测试已覆盖 Spike 两个 replay CLI、testnet URL
切换、combined stream 解包、订阅刷新、策略绑定和回测订单幂等；仍不能证明
测试网执行或实盘流程可用。
...
| **Campaign** | 全局互斥、逐币种状态、第一笔成交计时、恢复与终态 | Phase 2 |
...
| **回测可信度** | 无未来数据（✅已实现）、未完成 Kline 排除、部分成交、手续费、滑点、同秒顺序 | Phase 3 |
...
- `uv run --extra dev python -m pytest -q`：`39 passed`
- Python 编译检查通过
- Compose 配置解析通过
- 核心模块导入通过
